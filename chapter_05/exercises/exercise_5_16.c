...
int main(int argc, char *argv[])
{
	...
	if( (nlines = readlines(lineptr, MAXLINES)) >= 0) {
		...
		q_sort( (void**) lineptr, 0, nlines-1,
			(int (*)(void*,void*))(numeric ? numcmp : new_strcmp), reverse ? -1 : 1);
...
/*new_strcmp: compares s1 and s2 considering case, directory order if necessary */
int new_strcmp(char * s1,char * s2)
{
	if(directory) {
	...
	while(caseInsensitive ? (tolower(*s1) == tolower(*s2)) : (*s1 == *s2)) {
		if(*s1 == '\0')
			return 0;
		++s1;
		++s2;
	...
	return caseInsensitive ? (tolower(*s1) - tolower(*s2)) : (*s1 - *s2);
